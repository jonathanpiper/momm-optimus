...
const shrinkAndDownload = async ({
    ...
    let localMediaPath = ['.jpg', '.svg', '.png'].includes(extension)
        ? media.replace(IMAGE_URL_PREFIX, '')
        : media.replace(MEDIA_URL_PREFIX, '')
    localMediaPath = name
        ? `${name.replace(/[^A-Z0-9]/gi, '_').toLowerCase()}${localMediaPath.substring(
              localMediaPath.indexOf(extension)
          )}`
        : localMediaPath
    localMediaPath =
        ['', 'artifact'].indexOf(flag) === -1
            ? localMediaPath.substring(0, localMediaPath.indexOf(extension)) +
              '_' +
              flag +
              extension
            : localMediaPath
                            ...
                            ? await Promise.all(
                                  item.storyMedia.map(async (media: StoryMedia, i: number) => {
                                      if (media.image) {
                                          ...
                                          console.log(
                                              `Story media item ${media.caption} not correctly defined.`
                                          )
                                      }
                                      return media
                                  })
                ...
                content.items = await Promise.all(
                    content.items.map(async (item: Item) => {
                        item.artifactImages = item.artifactImages
                            ? await Promise.all(
                                  item.artifactImages.map(
                                      async (image: ArtifactImage, i: number) => {
                            ...
                            : []
                        if (item.artifactImages.length === 0)
                            console.warn(`Artifact ${item.artifactNumber} has no images defined.`)
                        return item
                    })
                )
            }
